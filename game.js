...
function spawnPipe(now) {
    ...
    // 아이템 (55% 확률, BOSS 가중치 높음)
    if (Math.random() < 0.55) {
        // SHIELD 25%, DOUBLE 25%, BOSS 50%
        const r    = Math.random();
        const type = r < 0.25 ? 'SHIELD' : r < 0.5 ? 'DOUBLE' : 'BOSS';
...
function drawBg() {
    ...
    g.addColorStop(0,   '#74C0E8');
    g.addColorStop(0.7, '#AED9F0');
    g.addColorStop(1,   '#D8EFF8');
    ctx.fillStyle = g;
...
function drawMenu() {
    ...
    bgG.addColorStop(0,   '#3A7BD5');
    bgG.addColorStop(0.5, '#74C0E8');
    bgG.addColorStop(1,   '#C8E8F8');
    ctx.fillStyle = bgG;
    ctx.fillRect(0, 0, W, H);
